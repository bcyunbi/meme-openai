const colors = ['#547fdb', '#42acce', '#CB958E', '#DDFFD9']
...
const p5background = (p5) => {
  p5.setup = () => {
    ...
    p5.noStroke()
    let vert = vertShaderSource
    let frag = fragShaderSource
    s = p5.createShader(vert, frag)
  }
  ...
  p5.draw = () => {
    ...
    s.setUniform('uResolution', [p5.windowWidth, p5.windowHeight])
    s.setUniform('uTime', p5.millis() / 100)
...
const fragShaderSource = `  #ifdef GL_ES
